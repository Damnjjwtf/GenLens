...
Promotion evidence is now machine-enforced rather than tracked in this document
alone. `state/lens_evaluations.json` stores dated run evidence and attributed
card-review events under a versioned schema. Same-day reruns cannot inflate the
streak, exact repeats reset it, and Marti/unified sends fail closed when the
evidence log is missing or incomplete. See `docs/PROMOTION_GOVERNANCE.md`.
